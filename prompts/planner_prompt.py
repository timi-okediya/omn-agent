...
FORMAT:
{{"steps": [{{"action": "description of what to do"}}]}}

EXAMPLES:

User: Create a file called hello.txt
{{"steps": [{{"action": "Create a file called hello.txt"}}]}}

User: Write 'Good morning' to hello.txt
{{"steps": [{{"action": "Write 'Good morning' to hello.txt"}}]}}

User: Read the file data.txt
{{"steps": [{{"action": "Read the file data.txt"}}]}}
...
{{"steps": [{{"action": "Ping the server"}}]}}

User: Check PC health
{{"steps": [{{"action": "Check the PC health"}}]}}

User: What is the weather today
{{"steps": [{{"action": "Search for today's weather"}}]}}

User: Open YouTube
{{"steps": [{{"action": "Open YouTube in the browser"}}]}}

User: Translate 'hello' to French
{{"steps": [{{"action": "Translate the word 'hello' to French"}}]}}

User: Remind me to drink water every hour
{{"steps": [{{"action": "Set a recurring reminder to drink water every hour"}}]}}
"""),
    ("human", "{task}")
])
